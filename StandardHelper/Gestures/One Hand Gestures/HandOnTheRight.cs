...
{
    public class HandOnTheRight : BodyGestureSegment
    {
        public override Enums.GesturePartResult checkGesture(Microsoft.Kinect.Skeleton skeleton)
        {
            if (handType != HandType.NULL)
            {
                ...
                {
                    if (skeleton.Joints[JointType.HandLeft].Position.X - skeleton.Joints[JointType.HipRight].Position.X > 0.1)
                    {
                ...
                {
                    if (skeleton.Joints[JointType.HandRight].Position.X - skeleton.Joints[JointType.HipRight].Position.X > 0.1)
                    {
        ...
        //constructor
        public HandOnTheRight(HandType handType = HandType.NULL)
        {
