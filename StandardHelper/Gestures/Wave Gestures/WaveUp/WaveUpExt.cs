...
{
    public class WaveUpExt : BodyGestureSegment
    {
        public override GesturePartResult checkGesture(Skeleton skeleton)
        {
            if (handType != HandType.NULL)
            {
                if (handType == HandType.LEFT)
                {
                    ...
                    {
                        //hand right of elbow
                        if (skeleton.Joints[JointType.ElbowLeft].Position.X - skeleton.Joints[JointType.HandLeft].Position.X > 0.1)
                        {
        ...
        //constructor
        public WaveUpExt(HandType handType = HandType.NULL)
        {
